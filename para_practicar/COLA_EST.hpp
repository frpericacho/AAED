#ifndef LISTA_EST_HPP
#define LISTA_EST_HPP
#include <cassert>
...
template <typename> class Lista{
    public:
        ...
        Lista<T>& operator =(const Lista<T>& l); //asignacionde listas
        void insertar(const T& x, posicion p);
        void eleiminar(posicion p);
        const T& elemento(posicoin p) const;
        T& elemento(posicion p);
        posicion buscar(const T& x) const;
        posicion siguiente(posicion p) const;
        posicion anterior(posicion p) const;
        posicion primera() const;
        posicoin fin() const; //poscion despues del ultimo
        ~Lista(); //Destructor
...
template <typename T> inline Lista<T>::Lista(size_t TamaMax) : 
...
template <typename T> Lista<T>& Lista<T>::operator =(const Lista<T>& l)
{
 ...
 template <typename T> inlineconst T& Lista<T>::elemento(Lista<T>::posicion p) const
 {
     assert(p >= 0 && p < n);
     return elementos[p];
 }
